...
// AI로 질문 생성 (무료 Hugging Face API 사용)
...
주제: ${categoryDescriptions[category.id]}
날짜 시드: ${todaySeed}
...
JSON 배열만 출력하고 다른 설명은 포함하지 마세요.`;
    ...
    try {
        ...
        // 20개가 아니면 fallback 질문 사용
        if (questions.length !== 20) {
            console.warn('질문이 20개가 아닙니다. Fallback 질문 사용');
            questions = getFallbackQuestions(category.id);
        }
    } catch (e) {
        console.error('JSON 파싱 실패, Fallback 질문 사용:', e);
        questions = getFallbackQuestions(category.id);
    }
}
